...
def preprocess_visec(hf_id="hustep-lab/ViSEC", output_dir="visec_dataset", seed=42, target_classes=None):
    ...
    ds = load_dataset(hf_id, split="train", download_mode="force_redownload").cast_column("path", Audio(decode=False))
    
    if 'accent' not in ds.column_names:
        print("CRITICAL WARNING: 'accent' column NOT FOUND in the dataset! Did you load the correct version?")
    ...
    # Check if all accents are -1
    num_missing_accents = sum(1 for s in filtered_samples if s['accent'] == -1)
    if num_missing_accents == len(filtered_samples):
        print(f"CRITICAL ERROR: ALL {len(filtered_samples)} filtered samples have accent=-1! The CSV files will be useless for accent training.")
    else:
        print(f"Valid accent samples: {len(filtered_samples) - num_missing_accents}/{len(filtered_samples)}")
