...
class ViSECDataset(Dataset):
    def __init__(self, hf_id="hustep-lab/ViSEC", split="train", target_classes=['happy', 'neutral', 'sad', 'angry'], sr=44100, target_size=(244, 244), augment=False, spec_augment_cfg=None, pitch_shift_cfg=None, time_shift_cfg=None, load_accent=False, csv_path=None, waveform_augment_cfg=None):
        ...
        if csv_path is not None and os.path.exists(csv_path):
            ...
            print(f"Loaded {len(self.indices)} local samples.")
            
            if self.load_accent:
                accent_counts = {}
                for _, _, acc in self.indices:
                    accent_counts[acc] = accent_counts.get(acc, 0) + 1
                print(f"Local Accent distribution: {accent_counts}")
                if accent_counts.get(-1, 0) == len(self.indices):
                    print("CRITICAL WARNING: ALL local CSV accent labels are -1! The accent loss will be 0.0 during training. You MUST re-run preprocess_visec.py!")
        else:
            self.is_local = False
            ...
            if self.load_accent and not has_accent:
                print("WARNING: load_accent is True, but 'accent' column is MISSING in the Hugging Face dataset!")
            ...
            for idx, (emo, accent_str) in enumerate(zip(emotions, accents)):
                if emo in self.target_classes:
                    ...
                    if has_accent and accent_str is not None:
                        # If accent is already an integer (0, 1, 2)
                        if isinstance(accent_str, (int, float)) or (isinstance(accent_str, str) and accent_str.isdigit()):
                            acc_int = int(accent_str)
                            if 0 <= acc_int < self.num_accent_classes:
                                accent_label = acc_int
                        else:
                            # Clean string (e.g., 'North', ' south ')
                            clean_accent = str(accent_str).strip().lower()
                            if clean_accent in self.accent_map:
                                accent_label = self.accent_map[clean_accent]
        ...
        if self.load_accent:
            ...
            if accent_counts.get(-1, 0) == len(self.indices):
                print("CRITICAL WARNING: ALL accent labels are -1! The accent loss will be 0.0 during training.")
